...
# This package ships its own version of libstdc++.so (and related libs),
# to ensure that the user gets a version that is at least as new as the version that we want.
# But if the user's system already has a newer version of libstdc++.so,
# then our version will be missing some symbols that their system expects.
# See, for example: https://github.com/ilastik/ilastik-build-conda/issues/24
# 
# The following command removes our copy of libstdc++.so if the user didn't need it. 
