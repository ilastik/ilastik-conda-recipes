...
def read_dylib_id(dylib_path):
    ...
    assert (
        len(id_commands) == 1
    ), "Exepected 1 or 0 LC_ID_DYLIB commands, but found {} in {}".format(
        len(id_commands), dylib_path
    )
...
def read_load_commands(dylib_path):
    ...
    try:
        ...
        commands = []
        for command_index, group in groupby(
            zip(command_indexes, lines), lambda (i, l): i
        ):
