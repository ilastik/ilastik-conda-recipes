...
def remove_rpath(
    dylib_path, make_relative_to="loader_path", executable_path=sys.executable
):
    """
    ...
    try:
        ...
        sys.stderr.write(
            "Error {} while calling otool -L {}".format(ex.returncode, dylib_path)
        )
        raise
    else:
        ...
        if make_relative_to == "executable_path":
            ...
            relative_rpaths = map(
                lambda rpath: os.path.relpath(rpath, executable_path), abs_rpaths
            )

            rpath_replacements = map(
                lambda rpath: "@executable_path/" + rpath, relative_rpaths
            )
        else:
            relative_rpaths = map(
                lambda rpath: os.path.relpath(rpath, os.path.split(dylib_path)[0]),
                abs_rpaths,
            )

            rpath_replacements = map(
                lambda rpath: "@loader_path/" + rpath, relative_rpaths
            )
...
if __name__ == "__main__":
    ...
    if not parsed_args.with_executable_path and not parsed_args.with_loader_path:
        ...
        print(
            "Assuming python for default --executable_path={}".format(
                parsed_args.executable_path
            )
        )
