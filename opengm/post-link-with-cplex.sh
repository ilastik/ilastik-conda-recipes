...
(
    ...
    if [ `uname` == Darwin ]; then
        install_name_tool -add_rpath "${CPLEX_LIB_DIR}"   "${INFERENCE_MODULE_SO}"
        ...
        ${PREFIX}/bin/patchelf-bigfile --set-rpath "$CONCERT_LIB_DIR:$CPLEX_LIB_DIR:"'$ORIGIN/../../../..' "${INFERENCE_MODULE_SO}"
    fi
)
